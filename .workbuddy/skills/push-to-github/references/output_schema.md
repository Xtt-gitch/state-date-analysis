...
| `note` | string | 特殊情形说明（如"跳过提交直接推送"） |
| `pushed_commits` | string[] | 跳过提交时，实际推送的提交列表 |
| `missing` | string[] | 路径校验失败的文件列表 |
| `error` | string | 失败原因 |
| `hint` | string | 失败时的建议 |
...
- **脚本不是事务性的**：`git add` 与 `git commit` 一旦执行就无法自动回滚。失败时先用 `git log --oneline -3` 确认提交是否已生成，再决定后续动作。
- **`--dry-run` 只预演命令**，不做任何写操作，可安全反复调用。
- **路径校验**：工作区中不存在且 `git ls-files` 也不认识的路径会被拒绝；若路径曾被跟踪（即本次是"删除文件"），则放行并正常记录删除。
- **暂存区为空**分两种：若有已提交但未推送的内容，脚本会跳过提交直接推送并返回 `note`；否则以退出码 2 报错。
